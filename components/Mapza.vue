        ...
        <div>
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="text-center mb-8">
                    <h2 class="text-3xl font-bold text-gray-900 mb-2">สำรวจคุณภาพอากาศและภัยพิบัติ</h2>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center w-full md:w-1/2 relative">
                        <!-- Input -->
                        <input v-model="searchTerm" type="text" placeholder="ค้นหาสถานที่หรือที่อยู่..."
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden relative">
                    ...
                    <div class="absolute top-20 right-5">
                        <div class="data-filter flex flex-col space-y-0">
                            <!-- Air Quality Filter -->
                            <label class="filter-label flex flex-col items-center cursor-pointer group relative">
                                <input type="checkbox" v-model="showAirQuality" @change="updateMapMarkers"
                                    class="hidden" />
                                <div class="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200"
                                    :class="showAirQuality
                            ...
                            <!-- Fire Filter -->
                            <label class="filter-label flex flex-col items-center cursor-pointer group relative">
                                <input type="checkbox" v-model="showFires" @change="updateMapMarkers" class="hidden" />
                                <div class="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200"
                                    :class="showFires
                            ...
                            <!-- Earthquake Filter -->
                            <label class="filter-label flex flex-col items-center cursor-pointer group relative">
                                <input type="checkbox" v-model="showEarthquakes" @change="updateMapMarkers"
                                    class="hidden" />
                                <div class="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200"
                                    :class="showEarthquakes
            ...
            <!-- Air Quality Banner -->
            <div id="banner" class="banner" v-if="showBanner && selectedLocation.type === 'air_quality'">
                <h3 class="banner-title">คุณภาพอากาศใน <span class="location-title">{{ selectedLocation.place }}</span>
                </h3>
                ...
                <div class="air-details">
                    ...
                    <span class="detail-item"
                        style="display: block; text-align: center;  font-size: 12px; color: #666;">
            ...
            <!-- Fire Banner -->
            <div id="banner" class="banner fire-banner" v-if="showBanner && selectedLocation.type === 'fire'">
                <h3 class="banner-title">จุดความร้อน <span class="location-title">{{ selectedLocation.place }}</span>
                </h3>
                <div class="fire-info">
                    <img class="fire-icon" src="/assets/images/fire_icon.png" alt="fire icon"
                        style="width: 60px; height: 60px;">
                    <div class="fire-details">
                        <span class="fire-intensity"
            ...
            <!-- Earthquake Banner -->
            <div id="banner" class="banner earthquake-banner"
                v-if="showBanner && selectedLocation.type === 'earthquake'">
                <h3 class="banner-title">แผ่นดินไหว <span class="location-title">{{ selectedLocation.place }}</span>
                </h3>
                <div class="earthquake-info">
                    <img class="earthquake-icon" src="/assets/images/earthquake_icon.png" alt="earthquake icon"
                        style="width: 60px; height: 60px;">
                ...
                <div class="earthquake-additional">
                    ...
                    <span class="detail-item" :style="{ color: getTsunamiColor(selectedLocation.tsunami) }">
                        <strong>สึนามิ:</strong> {{ selectedLocation.tsunami ? 'มีโอกาส' : 'ไม่มีโอกาส' }}
                    </span>
                    <span class="detail-item" style="display: block; text-align: center; font-size: 12px; color: #666;">
                        อัพเดทล่าสุด: {{ selectedLocation.updateTime }}
                    </span>
...
export default {
    ...
    methods: {
        ...
        async fetchAirQualityData() {
            try {
                ...
                this.devices = resData.response.filter(device =>
                    ...
                    && device.pm25 && device.humidity && device.temperature
                    && device.ddate === today
                );
            ...
            try {
                ...
                if (Array.isArray(fireData)) {
                    ...
                    if (fireData.data && Array.isArray(fireData.data)) {
                        ...
                        firesArray = Object.values(fireData).filter(item =>
                ...
                this.fires = firesArray.map((fire, index) => {
                    ...
                    return {
                        ...
                        latitude: lat,
                        longitude: lng,
                        place: fire.location || fire.place || fire.name || `จุดความร้อน ${index + 1}`,
                        intensity: this.getFireIntensity(brightness),
                        confidence: Math.round(confidence),
                        date: fire.date || fire.detection_date || new Date().toLocaleDateString('th-TH'),
                        area: fire.area || 'N/A',
            ...
            try {
                ...
                this.earthquakes = earthquakeData.features
                    .filter(quake => {
                        ...
                        return quake.properties.mag >= 2.0
                            && quakeDate >= sevenDaysAgo;
                    })
                    .map(quake => ({
        ...
        getFireIntensityColor(intensity) {
            switch (intensity) {
                case 'สูงมาก': return '#ff0000';
                case 'สูง': return '#ff6b00';
                case 'ปานกลาง': return '#ffa500';
                case 'ต่ำ': return '#ffff00';
        ...
        searchLocation() {
            ...
            const foundAirQuality = this.devices.find(device => device.place.toLowerCase().includes(term));
            const foundFire = this.fires.find(fire => fire.place.toLowerCase().includes(term));
            const foundEarthquake = this.earthquakes.find(quake => quake.place.toLowerCase().includes(term));
        ...
        getStatusColor(status) {
            switch (status) {
                case 'ดีมาก': return '#30b2fc';
                case 'ดี': return '#6dd951';
                case 'ปานกลาง': return '#e9db51';
                case 'แย่': return '#efa628';
                case 'อันตราย': return 'red';
