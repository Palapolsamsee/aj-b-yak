<template>
  ...
  >
    <header class="flex flex-col items-center gap-1 text-center">
      <span class="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-500">Air Quality</span>
      <h2 class="text-xl font-semibold text-gray-900">ตารางคุณภาพอากาศ</h2>
      <p class="text-sm text-gray-500">เรียงลำดับสถานีจากคุณภาพอากาศแย่ที่สุด → ดีที่สุด</p>
    </header>

    <div class="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white/60 p-4 sm:flex-row sm:items-center sm:justify-between">
      <form class="flex w-full flex-col gap-2 sm:flex-row sm:items-center" @submit.prevent="applySearch">
    ...
    <div v-if="loading" class="py-10 flex justify-center">
      <span class="text-sm text-gray-500 animate-pulse">กำลังโหลดข้อมูล...</span>
    </div>

    <div v-else-if="error" class="py-10 text-center">
      <p class="text-sm text-rose-600 font-medium">เกิดข้อผิดพลาดในการโหลดข้อมูล</p>
      <button
    ...
    <div v-else>
      <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white/70">
        <div class="overflow-x-auto">
          <table class="min-w-full text-xs sm:text-sm">
            <thead class="bg-gray-50/80 text-[11px] font-semibold uppercase tracking-wide text-gray-500 sm:text-xs">
              <tr>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-3 py-2 text-gray-800">
                <p class="font-medium text-gray-900">{{ item.place ?? "-" }}</p>
                <p class="text-[11px] text-gray-500">{{ item.address ?? "-" }}</p>
              </td>
              <td class="px-3 py-2 text-gray-700">{{ getProvince(item.address) }}</td>
              <td class="px-3 py-2 text-right">
                <span class="font-semibold" :class="getCategoryColor(item.rankingValue)">
              ...
              <td class="px-3 py-2 text-gray-700">
                <span class="inline-flex items-center gap-1 text-xs">
                  ...
                  <span class="uppercase tracking-wide text-gray-500">{{ trendLabel(item.trend) }}</span>
                </span>
              </td>
              <td class="px-3 py-2">
                <span
                  class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-[11px] font-medium"
                  :class="getCategoryChip(item.rankingValue)"
                >
                  <span class="h-2 w-2 rounded-full" :class="getCategoryDot(item.rankingValue)" />
      ...
      <div class="px-3 py-3 sm:px-5 sm:py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-xs text-gray-500 sm:text-sm">
        <p class="text-center sm:text-left">ทั้งหมด {{ totalFiltered }} รายการ · หน้า {{ currentPage }} / {{ totalPages }}</p>
        ...
        <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl">
          ...
          <div class="px-5 py-6 space-y-4 text-sm text-gray-600">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div v-if="availableYears.length" class="flex items-center gap-2 text-xs text-gray-500 sm:text-sm">
                <span>เลือกปี</span>
                <select
          ...
          <footer class="flex justify-end gap-2 border-t border-gray-100 px-5 py-4">
            <button
...
import { fetchColorRanges, type ColorRange } from "@/utils/api/colorRanges";
    ...
    .filter(
      (item) =>
        (item.status ?? "")
          .toString()
          .trim()
          .toLowerCase() === ACTIVE_STATUS && item.rankingValue !== null
    )
    .sort(
      (a, b) =>
...
const getCategoryColor = (value: number | string | null | undefined) => {
  const numeric = toNumber(value);
  if (numeric === null) return "text-gray-500";
  if (numeric <= 50) return "text-emerald-600";
  if (numeric <= 100) return "text-amber-600";
  return "text-rose-600";
};

const getCategoryChip = (value: number | string | null | undefined) => {
  const numeric = toNumber(value);
  if (numeric === null)
...
const getCategoryDot = (value: number | string | null | undefined) => {
  const numeric = toNumber(value);
  if (numeric === null) return "bg-gray-400";
  if (numeric <= 50) return "bg-emerald-400";
  if (numeric <= 100) return "bg-amber-400";
  return "bg-rose-400";
};

const trendIcon = (trend?: string | null) => {
  const normalized = (trend ?? "").toString().toLowerCase();
  if (normalized === "u") return "⬆️";
  if (normalized === "d") return "⬇️";
  if (normalized === "s") return "➡️";
  return "•";
};
