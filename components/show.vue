<template>
  <div class="max-8xl min-h-[80vh] bg-[#070d22] mx-auto p-4 rounded-lg shadow-lg">
    ...
    <div class="mb-4 relative">
      ...
      <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-600 w-5 h-5" fill="currentColor"
        viewBox="0 0 24 24">
      ...
      <div v-for="device in filteredDevices.slice(0,9)" :key="device.dvid" class="bg-white rounded-xl shadow-md p-6 cursor-pointer transition-transform duration-300 ease-in-out 
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="flex-1 space-y-2">
            ...
            <p class="font-semibold text-left mt-2"
              :class="{ 'text-green-600': device.status === 'Active', 'text-red-600': device.status === 'InActive' }">
    ...
    <div v-if="selectedDevice" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center"
      ...
      <div class="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        <p><strong>สถานะ:</strong>
          <span
            :class="{ 'text-green-600': selectedDevice.status === 'Active', 'text-red-600': selectedDevice.status === 'InActive' }">
            {{ selectedDevice.status }}
          </span>
        </p>
        <button @click="selectedDevice = null"
          class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-all active:scale-95">ปิด</button>
      </div>
    </div>
  </div>
</template>
