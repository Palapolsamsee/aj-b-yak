<template>
  ...
  <table class="w-full table-auto text-center text-sm text-gray-1200 border-collapse shadow-lg bg-gray-200"> 
        ...
        <tbody class="divide-y divide-gray-400">
          <tr v-for="(device, index) in filteredDevices.slice(0, 50)" :key="device.dvid" 
            ...
            <td @click="openDeviceModal(device)" class="px-6 py-4 text-gray-800 cursor-pointer whitespace-nowrap hover:underline">{{ device.place }}</td>
            ...
            <td class="px-6 py-4 text-gray-800 whitespace-nowrap relative">
              ...
              <transition name="fade">
                <div v-if="expandedRow === device.dvid" class="absolute left-0 mt-2 bg-gray-100 p-3 rounded-lg shadow-lg w-60 z-10 text-sm text-gray-800">
  ...
  <div class="bg-white p-6 rounded-lg shadow-xl max-w-md relative">
        ...
        <button @click="closeDeviceModal" class="absolute top-2 right-2 text-black hover:text-gray-600 px-3 py-1 rounded-full transition-all">✖</button>
      </div>
    </div>
  </div>
