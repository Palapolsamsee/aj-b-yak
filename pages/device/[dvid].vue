    ...
    <div v-else class="bg-white shadow-2xl rounded-3xl p-10 max-w-6xl mx-auto font-sans">
      ...
      <div class="flex items-center justify-between mb-10 flex-wrap">
        <div>
          ...
          <h5 class="text-xl font-extrabold text-gray-800">{{device.address}}</h5>
           </p>
        </div>
...
const preventionTips = computed(() => {
  ...
  if (lang.value === "th-TH") {
    if (value <= 25) {
      return ["สามารถทำกิจกรรมกลางแจ้งได้ตามปกติ", "ดูแลสุขภาพและออกกำลังกายได้ตามปกติ"]
    } else if (value <= 37) {
      return ["สามารถทำกิจกรรมกลางแจ้งได้ แต่ควรติดตามค่าฝุ่น", "ผู้ที่มีโรคประจำตัวควรระวังอาการผิดปกติ"]
    } else if (value <= 50) {
      return ["ลดเวลาในการทำกิจกรรมกลางแจ้ง", "ดื่มน้ำมาก ๆ และพักผ่อนให้เพียงพอ", "ผู้ที่มีโรคทางเดินหายใจควรพกยาประจำตัว"]
    } else if (value <= 90) {
      return ["หลีกเลี่ยงกิจกรรมกลางแจ้ง", "สวมหน้ากาก N95 เมื่อออกนอกบ้าน", "ปิดประตูหน้าต่างเพื่อป้องกันฝุ่นเข้าอาคาร"]
    }
    return ["งดกิจกรรมกลางแจ้งโดยเด็ดขาด", "สวมหน้ากาก N95 ตลอดเวลาที่อยู่นอกบ้าน", "ใช้เครื่องฟอกอากาศในบ้าน", "ติดตามประกาศจากหน่วยงานสาธารณสุข"]
  } else {
    if (value <= 25) {
      return ["You can do outdoor activities as usual", "Stay healthy and exercise regularly"]
    } else if (value <= 37) {
      return ["Outdoor activities are fine but monitor dust levels", "People with health issues should be cautious"]
    } else if (value <= 50) {
      return ["Reduce time outdoors", "Drink water and rest well", "Carry medicine if you have respiratory issues"]
    } else if (value <= 90) {
      return ["Avoid outdoor activities", "Wear an N95 mask when outside", "Keep doors and windows closed"]
    }
    return ["Completely avoid outdoor activities", "Always wear an N95 mask outside", "Use an air purifier indoors", "Follow public health announcements"]
  }
})

// Fetch Device Data by DVID
const fetchDeviceByDvid = async () => {
  try {
    ...
    const found = devices.find((dev: Device) => dev.dvid === dvid)
